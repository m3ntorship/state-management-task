...
const ImagePoll = (props) => {
  ...
  const [
    thereIsAtleastOneImageUploaded,
    setThereIsAtleastOneImageUploaded,
  ] = useState(false);
  useEffect(() => {
    imagesInfo.forEach((image) => {
      if (image.imageUploaded) {
        setThereIsAtleastOneImageUploaded(true);
      }
    });
  }, [imagesInfo]);
  return (
      ...
      {thereIsAtleastOneImageUploaded ? (
        imagesInfo.map((imagesInfo) => {
          if (imagesInfo.imageUploaded) {
            ...
            return <React.Fragment key={imagesInfo.imageId}></React.Fragment>;
          }
        })
      ) : (
        <h1>You gotta wait for the image to upload idiot</h1>
      )}
    </div>
  );
};
